import {
  ...
  WATER_LINE,
  YUZU_RAMP,
} from './capybara-palette';
import { NormalisedRect, coverRect, roundedPerimeter } from './stage-mapping';
...
export class CapybaraRenderer {
  ...
  private paintBody(): void {
    ...
    // A real outline now, matching the head/ears/muzzle — see paintHead's own
    // note on why: the reference this was redone against is proper line art,
    // and a body with an inked edge next to a head without one is what read as
    // two different drawings stitched together.
    ctx.strokeStyle = ONSEN.capyDark;
    ctx.lineWidth = 7;
    roundedRect(ctx, -152, top, 304, 240, 122);
    ctx.stroke();
  ...
  private paintHead(): void {
    ...
    // Redone from scratch against the reference sheet (see headPath's own
    // comment). The single biggest change from every earlier pass: a real,
    // solid, dark outline around every shape on the face, not a low-alpha
    // "wrap." Flat-filled shapes with no line work is what made every version
    // so far look like a soft blob regardless of its proportions — the
    // reference is proper line art, ink around every silhouette, and that
    // crispness is doing as much of the "this is a capybara" work as the
    // shapes themselves.
    ctx.fillStyle = ONSEN.capy;
    headPath(ctx, turn);
    ctx.fill();
  ...
  /**
   * Cheek patches flanking the muzzle shield.
   *
   * Present in every pose of the second reference sheet (the three-pose chibi
   * sheet in this project's root) — same warm tan as the muzzle, not a
   * reddish blush, sitting just outside it at roughly the same height. Drawn
   * first, before the muzzle, so the muzzle's own outline draws cleanly over
  ...
  private paintCheeks(turn: number): void {
    ...
    for (const side of [-1, 1] as const) {
      ctx.beginPath();
      ctx.ellipse(side * 92 + turn * 10, -26, 30, 26, 0, 0, Math.PI * 2);
  ...
  private paintEars(turn: number): void {
    ...
    // Barely there — in the second reference sheet the ears are essentially
    // invisible from the front, small enough to read as texture rather than a
    // feature. Close together near the top-centre, small enough that even a
    // full-size stroke would overpower them, so this one skips the outline.
    for (const side of [-1, 1] as const) {
      ctx.save();
      ctx.translate(side * 52 + turn * 4, -128);
      ctx.rotate(side * (0.22 - forward * 0.4 + (side > 0 ? twitch : -twitch * 0.6)));
      ctx.beginPath();
      ctx.ellipse(0, 0, 10, 9, side * 0.12, 0, Math.PI * 2);
      ctx.fillStyle = ONSEN.capy;
      ctx.fill();
      ctx.strokeStyle = ONSEN.capyDark;
      ctx.lineWidth = 3;
  ...
  /**
   * Muzzle, nose and mouth — the actual species tell.
   *
   * Redone again against a second, cleaner reference (the three-pose chibi
   * sheet, also dropped in this project's root): there the muzzle isn't a wide
   * shelf spanning the face — it's a contained oval "shield" sitting centred
   * in the lower-middle of the face, with matching cheek patches flanking it
   * (see paintCheeks). That combination — oval shield plus cheeks, all the
   * same warm tan, all outlined — is what that reference's face actually is.
   */
  private paintMuzzle(turn: number): void {
    ...
    ctx.beginPath();
    ctx.ellipse(mx, -12, 74, 68, 0, 0, Math.PI * 2);
    ctx.fillStyle = ONSEN.capyMuzzle;
    ctx.fill();
    ctx.strokeStyle = ONSEN.capyDark;
    ctx.lineWidth = 6;
    ...
    // Nose: a small dark oval near the top of the shield.
    ctx.fillStyle = ONSEN.ink;
    ctx.beginPath();
    ctx.ellipse(mx, -38, 14, 9, 0, 0, Math.PI * 2);
    ctx.fill();
  ...
  private paintEyes(turn: number, brow: number): void {
    ...
    for (const side of [-1, 1] as const) {
      const ex = side * 46 + turn * 12;
      // Close-set, right at the top edge of the snout — matching the reference
      // rather than a guessed "high adult proportion." It's the huge muzzle
      // below that carries the species, not the eye height.
      const ey = -62;
      const radius = 11;
 ...
 *
 * Redone from scratch against an actual reference sheet
 * (vecteezy_capybara-working-set, dropped in this project's root) instead of
 * being guessed and re-guessed. That reference's head is close to a rounded
 * square — noticeably wider than tall, but round on every corner, not flat
 * across the top and not tapered into a chin. A uniform, generous corner
 * radius on all four corners is what a rounded-square skull actually is;
 * mixing a small top radius with a large bottom one (the previous attempt)
 * drew a shield, not a head.
 *
 * Local units, same frame as everything else: 236 wide, 154 tall, sitting
 * from y = -128 to y = 26.
 */
function headPath(ctx: CanvasRenderingContext2D, turn: number): void {
  ...
  const top = -128;
  const bottom = 26;
  roundedRect(ctx, x - hw, top, hw * 2, bottom - top, 64);
}
